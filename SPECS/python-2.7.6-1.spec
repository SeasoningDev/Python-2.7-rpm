...
#%define include_docs %(if [ -f "%{_sourcedir}/%{name}-%{version}-docs-html.tar.bz2" ]; then echo 1; else echo 0; fi)
...
%changelog
* Thu Mar 17 2011 Keith Dart <keith.dart@thalesesec.com> [2.7-1TH]
- Fixed to build on CentOS 5.2 without conflicting with installed 2.4 Python.
...
%{__prefix}/bin/python%{libvers}-config
